...
namespace DME.Web.Template
{
    ...
    /// </summary>
    public class FunctionTag : DMEWeb_Tag
    {
        /// <summary>
        /// 
        /// </summary>
        /// <param name="ownerTemplate"></param>
        internal FunctionTag(DMEWeb_Template ownerTemplate)
            : base(ownerTemplate)
        {
            this.FunctionArgs = new DMEWeb_ElementCollection<DMEWeb_IExpression>();
        }
        ...
        /// 参与函数运算的参数列表
        /// </summary>
        public virtual DMEWeb_ElementCollection<DMEWeb_IExpression> FunctionArgs { get; protected set; }

        /// <summary>
        /// 调用的方法
        /// </summary>
        public DMEWeb_Attribute Method
        {
            get
            {
        ...
        private object GetFunctionResult()
        {
            ...
            foreach (DMEWeb_IExpression exp in this.FunctionArgs)
            {
                        ...
                        foreach (MethodInfo m in methods)
                        {
                            ...
                            else if (parameters.Length == funcParams.Count)
                            {
                                ...
                                for (var i = 0; i < parameters.Length; i++)
                                {
                                    ...
                                    {
                                        v = DMEWeb_Utility.ConvertTo(funcParams[i].ToString(), parameters[i].ParameterType);
        ...
        internal override bool ProcessBeginTag(DMEWeb_Template ownerTemplate, DMEWeb_Tag container, Stack<DMEWeb_Tag> tagStack, string text, ref Match match, bool isClosedTag)
        {
            ...
            if (this.Method == null || string.IsNullOrEmpty(this.Method.Text)) throw new DMEWeb_ParserException(string.Format("{0}标签中缺少method属性", this.TagName));
