...
namespace DME.Web.Template
{
    ...
    /// 实用类
    /// </summary>
    internal static class DMEWeb_Utility
    {
        /// <summary>
        /// 
        /// </summary>
        static DMEWeb_Utility()
        {
        ...
        internal static int CompareTo(object value1, object value2, out bool success)
        {
            ...
            if (isSystemType1)
            {
                ...
                {                    
                    ...
                    {
                        //枚举值.则特殊转换
                        value2 = DMEWeb_Utility.ConvertTo(value2.ToString(), type1);
                    }
                    else
                    {
                        if (value2 is IConvertible)
                        {
                            value2 = DMEWeb_Utility.ChangeType(value2, type1);
                        }
                        else
                        {
                            value2 = null;
            ...
            else if (isSystemType2)
            {
                ...
                if (type1 != type2)
                {
                    ...
                    {
                        //枚举值.则特殊转换
                        value1 = DMEWeb_Utility.ConvertTo(value1.ToString(), type2);
                    }
                    else
                    {
                        if (value1 is IConvertible)
                        {
                            value1 = DMEWeb_Utility.ChangeType(value1, type2);
                        }
                        else
                        {
                            value1 = null;
        ...
        /// <returns></returns>
        internal static DMEWeb_IfConditionCompareType GetIfConditionCompareType(string compareType)
        {
            DMEWeb_IfConditionCompareType icct = DMEWeb_IfConditionCompareType.Equal;
            if (!string.IsNullOrEmpty(compareType))
            {
                switch (compareType.Trim())
                {
                    case ">":
                        icct = DMEWeb_IfConditionCompareType.GT;
                        break;
                    case ">=":
                        icct = DMEWeb_IfConditionCompareType.GTAndEqual;
                        break;
                    case "<":
                        icct = DMEWeb_IfConditionCompareType.LT;
                        break;
                    case "<=":
                        icct = DMEWeb_IfConditionCompareType.LTAndEqual;
                        break;
                    case "<>":
                    case "!=":
                        icct = DMEWeb_IfConditionCompareType.UnEqual;
                        break;
                    default:
                        icct = DMEWeb_IfConditionCompareType.Equal;
                        break;
                }
            }
            return icct;
        ...
        internal static object GetPropertyValue(object container, string propName, out bool exist)
        {
            ...
            if (DMEWeb_Utility.IsInteger(propName))
            {
                #region 索引值部分
                //属性名只为数字.则取数组索引
                int index = DMEWeb_Utility.ConverToInt32(propName);
        ...
        /// <returns></returns>
        internal static string ResolveFilePath(DMEWeb_Tag tag, string fileName)
        {
            if (!string.IsNullOrEmpty(fileName) && fileName.IndexOf(":") == -1 && !fileName.StartsWith("\\\\"))
            {
                ...
                while (string.IsNullOrEmpty(referPath) && tag != null)
                {
                    ...
                    {
                        referPath = ((DMEWeb_Template)tag).File;
                    }
                    else if (tag is DMEWeb_IncludeTag)
                    {
                        referPath = ((DMEWeb_IncludeTag)tag).File;
                    }
                    tag = tag.Parent;
                }
