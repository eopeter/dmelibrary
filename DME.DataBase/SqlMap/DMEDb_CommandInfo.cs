...
namespace DME.DataBase.DataMap.SqlMap
{
    ...
    /// 命令信息，支持处理分页SQL语句
    /// </summary>
    public class DMEDb_CommandInfo
    {
            ...
            get {
                if (AllCount > 0 )
                {
                    if (_sqlPageText == null)
                        _sqlPageText = DMEDb_SQLPage.MakeSQLStringByPage(this.DataBaseType, _SqlText, "", this.PageSize, this.PageNumber, this.AllCount);
                    return _sqlPageText;
                }
                else
                    return _SqlText; 
        ...
        /// <remarks></remarks>
        public void SetParameterValue(string paramName, string paramValue, DMEDb_enumParamType paramType)
        {
            if (paramType == DMEDb_enumParamType.DataParameter)
            {
