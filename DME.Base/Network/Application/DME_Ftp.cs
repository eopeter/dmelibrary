...
namespace DME.Base.Network.Application
{
    ...
    public class DME_Ftp
    {
        ...
        /// 获得刚刚上传的文件的存储路径
        /// </summary>
       // public static string Str_New_File_Name;
        /// <summary>
        /// 当前工作目录
        /// </summary>
        ...
        {
            UploadFileAsync(LocalFullPath, Path.GetFileName(LocalFullPath), true);
            Response.Close();
        }
        /// <summary>
        /// 异步上传文件到FTP服务器
        ...
        public void UploadFileAsync(string LocalFullPath, string RemoteFileName)
        {
            ...
            UploadFileAsync(LocalFullPath, RemoteFileName, true);
        }
        /// <summary>
        /// 异步上传文件到FTP服务器
        /// </summary>
        ...
        public void UploadFileAsync(string LocalFullPath, string RemoteFileName, bool OverWriteRemoteFile)
        {
            try
            {
                ...
                if (File.Exists(LocalFullPath))
                {
                    ...
                    //Day Date_Time = new Day();
                   // RemoteFileName = Date_Time.Get_Millisecond1() + "." + RemoteFileName.Substring(RemoteFileName.LastIndexOf(".") + 1);
                    client.UploadFileAsync(new Uri(this.Uri.ToString() + RemoteFileName), LocalFullPath);
                    //Str_New_File_Name = this.Uri.ToString() + RemoteFileName;
                }
                else
                {
                    throw new Exception("本地文件不存在!");
        ...
        public void UploadFileAsync(byte[] FileBytes, string RemoteFileName)
        {
            ...
            UploadFileAsync(FileBytes, RemoteFileName, true);
        }
        /// <summary>
        /// 异步上传文件到FTP服务器
        /// </summary>
