...
namespace DME.Web.Template.Evaluator
{
    ...
    /// 表达式计算器
    /// </summary>
    public class DMEWeb_ExpressionEvaluator
    {
        #region 构造函数
        /// <summary>
        /// 
        /// </summary>
        private DMEWeb_ExpressionEvaluator()
        {
        }
        #endregion
        ...
        private static List<DMEWeb_ExpressionNode> ParseExpression(string expression)
        {
            if (string.IsNullOrEmpty(expression)) return new List<DMEWeb_ExpressionNode>();

            List<DMEWeb_ExpressionNode> listOperator = new List<DMEWeb_ExpressionNode>(10);
            Stack<DMEWeb_ExpressionNode> stackOperator = new Stack<DMEWeb_ExpressionNode>(5);

            DMEWeb_ExpressionParser expParser = new DMEWeb_ExpressionParser(expression);
            DMEWeb_ExpressionNode beforeExpNode = null;       //前一个节点            
            DMEWeb_ExpressionNode unitaryNode = null;         //一元操作符
            DMEWeb_ExpressionNode expNode;
            ...
            {
                if (expNode.Type == DMEWeb_ExpressionNodeType.Numeric)
                {
                    //操作数， 直接加入后缀表达式中
                    if (unitaryNode != null)
                    {
                ...
                else if (expNode.Type == DMEWeb_ExpressionNodeType.LParentheses)
                {
                    //左括号， 直接加入操作符栈
                    stackOperator.Push(expNode);
                }
                else if (expNode.Type == DMEWeb_ExpressionNodeType.RParentheses)
                {
                    //右括号则在操作符栈中反向搜索，直到遇到匹配的左括号为止，将中间的操作符依次加到后缀表达式中。
                    DMEWeb_ExpressionNode lpNode = null;
                    while (stackOperator.Count > 0)
                    {
                        lpNode = stackOperator.Pop();
                        if (lpNode.Type == DMEWeb_ExpressionNodeType.LParentheses) break;
                        listOperator.Add(lpNode);
                    }
                    if (lpNode == null || lpNode.Type != DMEWeb_ExpressionNodeType.LParentheses)
                    {
                        throw new DMEWeb_ExpressionException(string.Format("在表达式\"{0}\"中没有与在位置({1})上\")\"匹配的\"(\"字符!", expParser.Expression, expParser.Position));
                    }
                }
                else
                {
                    if (stackOperator.Count == 0)
                    {
                        ...
                        if (listOperator.Count == 0 &&
                            !(expNode.Type == DMEWeb_ExpressionNodeType.LParentheses || expNode.Type == DMEWeb_ExpressionNodeType.Not))
                        {
                            //后缀表达式没有任何数据则判断是否是一元操作数
                            if (DMEWeb_ExpressionNode.IsUnitaryNode(expNode.Type))
                            {
                                unitaryNode = expNode;
                            }
                            else
                            {
                                //丢失操作数
                                throw new DMEWeb_ExpressionException(string.Format("表达式\"{0}\"在位置({1})上缺少操作数!", expParser.Expression, expParser.Position));
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        if (requireOperand)
                        {
                            ...
                            if (DMEWeb_ExpressionNode.IsUnitaryNode(expNode.Type) && unitaryNode == null)
                            {
                                unitaryNode = expNode;
                            }
                            else
                        ...
                        else
                        {
                            ...
                            do
                            {
                                ...
                                if (beforeExpNode.Type != DMEWeb_ExpressionNodeType.LParentheses && (beforeExpNode.PRI - expNode.PRI) >= 0)
                                {
            ...
            while (stackOperator.Count > 0)
            {
                ...
                if (beforeExpNode.Type == DMEWeb_ExpressionNodeType.LParentheses)
                {
                    throw new DMEWeb_ExpressionException(string.Format("表达式\"{0}\"中括号不匹配,丢失右括号!", expParser.Expression, expParser.Position));
                }
        ...
        /// <returns></returns>
        private static object CalcExpression(List<DMEWeb_ExpressionNode> nodes)
        {
            ...
            if (nodes.Count > 1)
            {
                ...
                {
                    DMEWeb_ExpressionNode node = nodes[index];

                    switch (node.Type)
                    {
                        //如果是数字，则将值存入 values 中
                        case DMEWeb_ExpressionNodeType.Numeric:
                        ...
                        default:
                            ...
                            int paramCount = 2;
                            if (node.Type == DMEWeb_ExpressionNodeType.Not) paramCount = 1;
                            //计算操作数的值
                            if (values.Count < paramCount)
                            {
                                throw new DMEWeb_ExpressionException("缺少操作数");
                            }
                            //传入参数
                            ...
                            node.Type = DMEWeb_ExpressionNodeType.Numeric;
                            //将操作数节点删除
                            for (int i = 0; i < paramCount; i++)
                            {
            ...
            {
                throw new DMEWeb_ExpressionException("缺少操作符或操作数");
            }
            #endregion
        }
        ...
        /// <returns></returns>
        private static object Calculate(DMEWeb_ExpressionNodeType nodeType, object[] data)
        {
            decimal d1, d2;
            bool b1, b2;
            switch (nodeType)
