...
namespace DME.Web.Template
{
    ...
    /// </summary>
    public class DMEWeb_VariableExpression : DMEWeb_IExpression
    {
        /// <summary>
        /// 变量表达式
        /// </summary>
        ...
        /// 变量标识
        /// </summary>
        public DMEWeb_VariableIdentity VariableId { get; private set; }
        /// <summary>
        /// 字段名
        /// </summary>
        ...
        /// 取得父级表达式
        /// </summary>
        private DMEWeb_VariableExpression ParentExpression { get; set; }
        /// <summary>
        /// 取得下一个表达式
        /// </summary>
        private DMEWeb_VariableExpression NextExpression { get; set; }
        ...
        {
            if (DMEWeb_Utility.IsNothing(data) && this.VariableId.Variable.GetCacheCount() == 0) return data;
            ...
            if (!string.IsNullOrEmpty(this.FieldName))
            {
                ...
                DMEWeb_VariableExpression exp = this;
                while (exp!= null)
                {
                ...
                if (!exist && !DMEWeb_Utility.IsNothing(data))
                {
                    //缓存数据不存在.则从实体取数据
                    if (this.IsMethod)
                    {
                        value = DMEWeb_Utility.GetMethodResult(data, this.FieldName, out exist);
                    }
                    else
                    {
                        value = DMEWeb_Utility.GetPropertyValue(data, this.FieldName, out exist);
                    }
                    //如果需要的话就缓存数据
