...
namespace DME.Web.Template
{
    ...
    /// </summary>
    public class DMEWeb_Template : DMEWeb_Tag
    {
        /// <summary>
        /// 
        /// </summary>
        internal DMEWeb_Template() : this(null) { }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// 自定义函数集合
        /// </summary>
        public DMEWeb_UserDefinedFunctionCollection UserDefinedFunctions { get; private set; }

        /// <summary>
        /// 返回此模板下的子模板元素
        /// </summary>
        public DMEWeb_ElementCollection<DMEWeb_Template> ChildTemplates { get; set; }

        /// <summary>
        /// 标签容器
        /// </summary>
        protected DMEWeb_Template TagContainer { get; set; }

        /// <summary>
        /// 返回处理模板数据的实例
        /// </summary>
        public DMEWeb_Attribute RenderInstance
        {
            get
            {
        ...
        /// 返回处理模板数据的特性方法
        /// </summary>
        public DMEWeb_Attribute RenderMethod
        {
            get
            {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public DMEWeb_Template GetChildTemplateById(string id)
        {
            if (string.IsNullOrEmpty(id)) throw new ArgumentNullException("id");
            foreach (DMEWeb_Template template in TagContainer.ChildTemplates)
            {
                ...
                {
                    DMEWeb_Template t = template.GetChildTemplateById(id);
                    if (t != null) return t;
                }
            }
            return null;
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public DMEWeb_ElementCollection<DMEWeb_Template> GetChildTemplatesByName(string name)
        {
            if (string.IsNullOrEmpty(name)) throw new ArgumentNullException("name");

            DMEWeb_ElementCollection<DMEWeb_Template> items = new DMEWeb_ElementCollection<DMEWeb_Template>();
            foreach (DMEWeb_Template template in TagContainer.ChildTemplates)
            {
        ...
        protected override void RenderTagData(System.IO.TextWriter writer)
        {
            ...
            if (!string.IsNullOrEmpty(renderInstance))
            {
                ...
                {
                    //采用特性方法处理
                    DMEWeb_Utility.PreRenderTemplateByAttributeMethod(renderInstance, renderMethod, this);
                }
                else
                {
                    //采用接口处理
                    DMEWeb_Utility.PreRenderTemplate(renderInstance, this);
                }
            }
            if (this.Visible)
            {
        ...
        internal override bool ProcessBeginTag(DMEWeb_Template ownerTemplate, DMEWeb_Tag container, Stack<DMEWeb_Tag> tagStack, string text, ref Match match, bool isClosedTag)
        {
            ...
            if (!string.IsNullOrEmpty(this.File))
            {
                ...
                if (System.IO.File.Exists(this.File))
                {
                    ...
                    //读取文件数据进行解析
                    new DMEWeb_TemplateDocument(this, System.IO.File.ReadAllText(this.File, this.Charset), ownerTemplate.OwnerDocument.DocumentConfig);
                }
            }
        ...
        {
            DMEWeb_Template tag = new DMEWeb_Template(ownerTemplate);
            //加入到宿主模板的子模板列表
