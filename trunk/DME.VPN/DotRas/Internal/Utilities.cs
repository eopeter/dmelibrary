...
namespace DotRas.Internal
{
    ...
    internal static class Utilities
    {
        ...
        /// <param name="value">The enum value to look locate.</param>
...
#pragma warning restore 0618
        ...
        {
            return address == null || address == IPAddress.Any;
        }
        ...
        public static Collection<IPAddress> CreateIPv4AddressCollection(IntPtr ptr, int count)
        {
            ...
            if (count > 0)
            {
                ...
                for (int index = 0; index < count; index++)
                {
                    ...
                    NativeMethods.RASIPADDR address = (NativeMethods.RASIPADDR)Marshal.PtrToStructure(current, typeof(NativeMethods.RASIPADDR));
        ...
        public static Collection<IPAddress> CreateIPv6AddressCollection(IntPtr ptr, int count)
        {
            ...
            if (count > 0)
            {
                ...
                for (int index = 0; index < count; index++)
                {
                    ...
                    NativeMethods.RASIPV6ADDR address = (NativeMethods.RASIPV6ADDR)Marshal.PtrToStructure(current, typeof(NativeMethods.RASIPV6ADDR));
                    retval.Add(new IPAddress(address.addr));
                }
            }
