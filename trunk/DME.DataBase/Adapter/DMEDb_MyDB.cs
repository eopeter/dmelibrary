...
namespace DME.DataBase.DataProvider.Adapter
{
    ...
    public class DMEDb_MyDB
    {
        ...
        /// 也可以直接使用 下面的形式：
        /// providerName="SqlServer" ，当然，这种形式的提供程序程序集默认就是 DME.DataBase.CommonDataProvider.Data 。
        /// 如果有多个，默认取 name="default"
        /// </summary>
        ...
        public static DMEDb_AdoHelper GetDBHelper()
        {
            ...
            if (DME_Validation.IsNull(engineType))
            {
                ...
                {
                    throw new Exception("未指明【EngineType】配置键，也未在【DME_DataBase_Connection】配置节【default】配置连接信息");
                }
            }
            else
            {
