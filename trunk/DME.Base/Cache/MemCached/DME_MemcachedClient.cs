...
namespace DME.Base.Cache.MemCached
{
    ...
    /// </summary>
    public class DME_MemcachedClient
    {
        #region Static fields and methods.
        private static Dictionary<string, DME_MemcachedClient> instances = new Dictionary<string, DME_MemcachedClient>();
        private static DME_LogAdapter logger = DME_LogAdapter.GetLogger(typeof(DME_MemcachedClient));
		...
		public static void Setup(string name, string[] servers) {
            ...
            instances[name] = new DME_MemcachedClient(name, servers);
		}
		...
		/// </summary>
        private static DME_MemcachedClient defaultInstance = null;
        public static DME_MemcachedClient GetInstance()
        {
        ...
        public static DME_MemcachedClient GetInstance(string name)
        {
            DME_MemcachedClient c;
			if (instances.TryGetValue(name, out c)) {
				return c;
			} else {
                DME_MemCachedConfig config = DME_MemCachedSection.GetConfig();

				if (config != null && config.Nodes.ContainsKey(name)) {
					Setup(name, config.Nodes[name].Hosts.Split(new char[] { ',' }));
                    c = GetInstance(name);
                    c.IsCache = config.IsCache;
                    c.KeyPrefix = config.Nodes[name].KeyPrefix;
                    c.SendReceiveTimeout = config.SendReceiveTimeout;
                    c.MinPoolSize = (uint)config.MinPoolSize;
                    c.MaxPoolSize = (uint)config.MaxPoolSize;
		...
		private readonly DME_ServerPool serverPool;
		...
		private string store(string command, string key, bool keyIsChecked, object value, uint hash, int expiry, ulong unique) {
			...
			return serverPool.Execute<string>(hash, "", delegate(DME_PooledSocket socket) {
				DME_SerializedType type;
				byte[] bytes;

				//Serialize object efficiently, store the datatype marker in the flags property.
				try {
					bytes = DME_Serializer.Serialize(value, out type, CompressionThreshold);
				} catch (Exception e) {
		...
		private object get(string command, string key, bool keyIsChecked, uint hash, out ulong unique) {
			...
			ulong __unique = 0;
			object value = serverPool.Execute<object>(hash, null, delegate(DME_PooledSocket socket) {
		...
		private object[] get(string command, string[] keys, bool keysAreChecked, uint[] hashes, out ulong[] uniques) {
			...
			Dictionary<DME_SocketPool, Dictionary<string, List<int>>> dict = new Dictionary<DME_SocketPool, Dictionary<string, List<int>>>();
			for (int i = 0; i < keys.Length; i++) {
				Dictionary<string, List<int>> getsForServer;
				DME_SocketPool pool = serverPool.GetSocketPool(hashes[i]);
			...
			foreach (KeyValuePair<DME_SocketPool, Dictionary<string, List<int>>> kv in dict) {
				serverPool.Execute(kv.Key, delegate(DME_PooledSocket socket){
					...
					while (readValue(socket, out gottenObject, out gottenKey, out unique)) {
						foreach(int position in kv.Value[gottenKey.Substring(keyPrefix.Length)]) {
		...
		/// </summary>
		public bool Remove(string key) { return delete(key, true, hash(key), 0); }
		public bool Remove(string key, uint hash) { return delete(key, false, this.hash(hash), 0); }
		public bool Remove(string key, TimeSpan delay) { return delete(key, true, hash(key), (int)delay.TotalSeconds); }
		public bool Remove(string key, uint hash, TimeSpan delay) { return delete(key, false, this.hash(hash), (int)delay.TotalSeconds); }
		public bool Remove(string key, DateTime delay) { return delete(key, true, hash(key), getUnixTime(delay)); }
		public bool Remove(string key, uint hash, DateTime delay) { return delete(key, false, this.hash(hash), getUnixTime(delay)); }
		...
		private bool delete(string key, bool keyIsChecked, uint hash, int time) {
			...
			return serverPool.Execute<bool>(hash, false, delegate(DME_PooledSocket socket){
				string commandline;
				if (time == 0) {
		...
		private ulong? incrementDecrement(string cmd, string key, bool keyIsChecked, ulong value, uint hash) {
			...
			return serverPool.Execute<ulong?>(hash, null, delegate(DME_PooledSocket socket) {
		...
		/// </summary>
		public bool RemoveAll() { return RemoveAll(TimeSpan.Zero, false); }
		public bool RemoveAll(TimeSpan delay) { return RemoveAll(delay, false); }
		public bool RemoveAll(TimeSpan delay, bool staggered) {
            if (!IsCache)
            {
                return true;
            }
			...
			bool noerrors = true;
			uint count = 0;
            foreach (DME_SocketPool pool in serverPool.HostList)
            {
